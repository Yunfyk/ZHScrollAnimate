...
- (void)viewDidLoad {
    ...
    self.titles = @[@"人生若只如初见",@"何事秋风悲画扇",@"等闲变却故人心",@"却道故人心易变",@"骊山语罢清宵半",@"泪雨霖铃终不怨",@"何如薄幸锦衣郎",@"比翼连枝当日愿"];
...
//    self.scrollView.autoAnimate     = YES;
    ...
    NSArray *HC = [NSLayoutConstraint constraintsWithVisualFormat:@"H:|-100-[scrollView(130)]" options:NSLayoutFormatAlignAllCenterX metrics:nil views:@{@"scrollView":self.scrollView}];
    ...
    self.scrollView.contentBlock = ^UIView *(NSInteger index){
        ...
        label.text = wkSelf.titles[index%wkSelf.titles.count];
        return label;
    };
